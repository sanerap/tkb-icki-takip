...
window.renderMainContent = function() {
    ...
    if (window.isAdmin) {
        ...
        html += `<div style="margin-top: 20px; padding: 15px; background: rgba(74, 144, 226, 0.1); border: 1px solid #4a90e2; border-radius: 8px; text-align: center;"><div style="color: #4a90e2; font-weight: 600; font-size: 18px;">${window.currentSession}</div><div style="color: #4a90e2;">${window.sessionData.participants[window.currentParticipantId]?.name || 'Katılımcı'}</div></div><div style="margin-top:10px;"><button class="btn-primary" onclick="window.openAddFoodModal()" style="width:100%; background:#e2a84a; color:#1a1a2e; margin-bottom:10px;">🍕 Ortak Yiyecek Ekle</button><button class="btn-primary" onclick="window.showQRCode()" style="width:100%;">📱 QR Kod Paylaş</button></div><button class="btn-primary btn-secondary" onclick="window.leaveSession()" style="margin-top:20px; width:100%;">Oturumdan Ayrıl</button>`;
    }
